...
- (void)viewDidLoad {
  ...
  self.feedStore = [[FeedStore alloc] init];
  [self.feedStore buildData];
}
...
- (void)addItem
{
  ...
  self.confirmAction = [UIAlertAction actionWithTitle:@"提交" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
    ...
    [self.feedStore addFeed:feed atIndex:0];
  }];
...
{
  if (self.feedStore.count > index) {
    [self.feedStore removeAtIndex:index];
  }
}
...
{
  return (NSInteger)self.feedStore.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  if (_isUseXib) {
    ...
    [(AdaptiveTableViewCell *)cell setFeed:[self.feedStore feed:indexPath.row]];
  } else {
    cell = [tableView dequeueReusableCellWithIdentifier:cellID forIndexPath:indexPath];
    if (cell == nil) {
      cell = [[PureCodeTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
    }
    [(PureCodeTableViewCell *)cell bindData:[self.feedStore feed:indexPath.row]];
  }
